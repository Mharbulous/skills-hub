...
- **If skip:** announce: "Both versions kept on disk. Run
  `/determinize2 -test` in a new session to compare them later."
...
currently reading — i.e. this `determinize2` (or `determinize`, once
promoted) skill's own root directory. Resolve it from where you are, don't
hardcode it.
...
Announce: "Both versions retained. Run `/determinize2 -test` in a new
