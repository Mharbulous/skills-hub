...
Data format and handling details for Telus Business Connect integration. Load this file when parsing Telus CSV data or interpreting telephony activities.
...
Telus Business Connect tracks more than just calls:
...
- Telus entries → `activity_type: "phone_call"` (covers calls, voicemail, text, fax — all telephony activity)
- Signals = phone number + direction + duration + contact name + activity subtype (call/voicemail/text/fax)

## Source Hierarchy

Telus Business Connect is the most reliable source for what the practitioner was actually doing. When Telus overlaps with another source, first try to reconcile (e.g., a TimeCamp window showing "Smith v Jones" during a Telus call → both signals agree on the matter). Only apply precedence when signals genuinely conflict.

### Telus Activity During TimeCamp Activity

If irreconcilable: the Telus activity replaces the overlapping portion. Split the TimeCamp activity into up to three segments:
- Pre-activity segment (TimeCamp, before Telus start)
- Telus segment (Telus — keep TimeCamp window titles during this period as attribution signals)
- Post-activity segment (TimeCamp, after Telus end)

Drop any resulting segment shorter than 1 minute.

### Telus Activity During GCal Event

If irreconcilable: Telus takes precedence for activity type. Calendar event context preserved as attribution signal.

## Task Assignment Context

For Telus activities, check TimeCamp window titles during the same period. If the practitioner had a client file open during the call, assign to that matter's Task with `confidence: "medium"`.

## Raw Data Storage

```bash
cowork-db set timesort "raw:telus:{YYYY-MM-DD}" '{json_array}'
```

Record `"telus"` in `sources_fetched`.
