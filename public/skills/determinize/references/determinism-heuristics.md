...
## Worked example: scoring the find-matches fixture

This walks the scoring process against `tests/sample-find-matches-skill.md`
(the fixture used in Scenario 5) so the abstract rubric above has a concrete
calibration point. Two phases are worked in full below; score Phases 2, 3,
and 4 the same way during actual use.

### Phase 1: Normalization (7-rule fixed transformation table)

| Heuristic | Score | Why |
|---|---|---|
| Computational Intensity | 2 | 7 rules applied per field, across every field of every record |
| Data Volume | 0 | Every character of the field is processed — nothing is filtered out |
| Iteration Over Collections | 3 | Unbounded — runs over every field of every record supplied |
| Precision-Critical | 3 | Exact punctuation stripping, exact accent substitution, exact suffix matching |
| State Accumulation | 0 | Each field is normalized independently; nothing carries over |
| File I/O | 0 | No file or directory reads |
| Cross-Referencing | 0 | No comparison against a second dataset (that's Phase 3) |
| Format Transformation | 2 | Structured text-to-text transformation |
| Multi-Step Pipelines | 3 | 7 sequential rules, applied in a fixed order |
| Redundant Computation | 3 | The same 7-rule sequence repeats for every field of every record |
| Enumerable Branching | 0 | No conditional branches — straight-line rule application |

**Total: 16/33 → Medium priority.**

Section line count: 17. Execution frequency: 1 (the field-level looping is
already captured by the Iteration heuristic above, not by frequency).

Determinism Value = (16/33) × 17 × 1 ≈ 8.2

### Phase 5: Keyword Extraction (6-step fixed pipeline)

| Heuristic | Score | Why |
|---|---|---|
| Computational Intensity | 3 | Tokenizing, counting, and sorting across every surviving record |
| Data Volume | 2 | Drops short tokens and stopwords — a meaningful fraction is discarded |
| Iteration Over Collections | 3 | Runs over every token of every surviving Phase 4 record |
| Precision-Critical | 3 | Exact length threshold, exact stopword membership, exact frequency count |
| State Accumulation | 2 | Token-frequency counts accumulate across all surviving records |
| File I/O | 0 | No file or directory reads |
| Cross-Referencing | 0 | No comparison against a second dataset |
| Format Transformation | 3 | Free text → ranked, deduplicated keyword list |
| Multi-Step Pipelines | 3 | 6 sequential steps, each feeding the next |
| Redundant Computation | 1 | Length/stopword filtering repeats per token, already priced into Iteration |
| Enumerable Branching | 3 | Length cutoff, stopword membership, and the frequency/alphabetical tie-break are all fully specified |

**Total: 26/33 → High priority.**

Section line count: 16. Execution frequency: 1.

Determinism Value = (26/33) × 16 × 1 ≈ 12.6

Phase 5 outranks Phase 1 despite one fewer line, because it scores higher
on Iteration, Multi-Step Pipelines, Format Transformation, and Enumerable
Branching — a 6-step pipeline ending in an explicit tie-break rule beats a
straight-line 7-rule substitution table.
