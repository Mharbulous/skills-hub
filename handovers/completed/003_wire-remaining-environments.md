...
**Done:**
- Firebase Hosting live at https://mharbulous.github.io/skills-hub/
...
**Remaining:**
- Claude sandboxes (web/Desktop): configure environment setup scripts to run `curl -fsSL "https://mharbulous.github.io/skills-hub/claude/skills.tar.gz" | tar -xz -C ~/.claude/skills` — this MUST be a setup script, NOT a SessionStart hook
- Claude CLI local: run `SKILLS_BASE_URL="https://mharbulous.github.io/skills-hub" ./bootstrap/claude-setup.sh` or set up a local symlink from dist/
- Codex: confirm skills directory location, run `SKILLS_BASE_URL="https://mharbulous.github.io/skills-hub" ./bootstrap/codex-setup.sh`
- E2E test: verify SyncoPaid web session has `commit` skill with `gather-context.sh` and `sanitize-commit.sh` working
- Add `mharbulous.github.io/skills-hub` to sandbox network allowlists if needed

## Key Discoveries

- Skills tarball verified accessible at https://mharbulous.github.io/skills-hub/claude/skills.tar.gz (HTTP 200)
