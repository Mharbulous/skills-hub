...
| Setting | Location in settings.json | Flag if | Recommended |
|---------|--------------------------|---------|-------------|
| `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` | `env` block (string) | Missing or > 80 | `"75"` |
| `BASH_MAX_OUTPUT_LENGTH` | `env` block (string) | At default (30-50K) | `"150000"` |

Both are environment variables in the `env` object. Example fix:
```json
"env": {
...
```
Note: values above ~83 are silently clamped — this setting can only trigger compaction *earlier*, not later.
...
| Missing CLAUDE_AUTOCOMPACT_PCT_OVERRIDE in env | -10 |
| Missing BASH_MAX_OUTPUT_LENGTH in env | -5 |
...
- Add the missing env vars to settings.json
